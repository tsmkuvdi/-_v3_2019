...
//	$sql = "SELECT * FROM $dbtablename where boutainame like '$aimaiken'";
	$sql = "SELECT * FROM $dbtablename AS t1 JOIN table_shokuin AS t2 ON t1.shokuinbangou1 = t2.bangou where boutainame like '$aimaiken' ORDER BY kensaday";
	$stmt = $dbh->query($sql);
	$result = $stmt->fetchAll(PDO::FETCH_ASSOC);
	echo "<table border=1 cellspacing=1> \n";
	echo "<tr>\n";
	echo "<tr>\n";
	echo "<th>詳細・印刷/入力・編集</th><th>管轄</th><th>防火対象物名</th><th>令別表</th><th>状況</th><th>不備その他内容</th><th>立入検査日</th><th>主担当名</th><th>副担当名</th><th>改修計画提出期限</th><th>改修計画提出日</th><th>改修計画完了期限</th><th>改修報告提出日</th><th>連絡1</th><th></th>\n";
	echo "</tr>\n";
	foreach ($result as $row) {
		echo "<td>\n";
		echo "<a href=../shousai.php?id=" . htmlspecialchars($row['id_kensa'],ENT_QUOTES,'UTF-8') . " >詳細</a>\n";
		echo "<a href=../kensaedit.php?id=" . htmlspecialchars($row['id_kensa'],ENT_QUOTES,'UTF-8') . ">入力</a>\n";
		echo "</td>\n";
...
//		echo "<td>" . htmlspecialchars($row['shokuinbangou1'],ENT_QUOTES,'UTF-8') . "</td>\n";
		...
		echo "<td>" . htmlspecialchars($row['ren1'],ENT_QUOTES,'UTF-8') . "</td>\n";
