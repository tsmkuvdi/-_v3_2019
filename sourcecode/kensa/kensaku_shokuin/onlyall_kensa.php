...
<h3>
 <table  width=100%>
  <tr>
    <div align="left"><a href="../kensaku_shokuin/shutan_preken.php">職員別立入検査主担当件数一覧に戻る</a></div>
   </th>
  </tr>
 <table>
</h3>
...
try {
	if (empty($_GET['shokuinbangou1'])) throw new Exception('Error');
	$bangou = (int) $_GET['shokuinbangou1'];
	$dbh = new PDO("mysql:host=localhost;dbname=$databasename;charset=utf8", $user, $pass);
	$dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$sql = "SELECT * FROM $dbtablename AS t1 JOIN table_shokuin AS t2 ON t1.shokuinbangou1 = t2.bangou where shokuinbangou1 = $bangou ORDER BY kensaday";
	$stmt = $dbh->query($sql);
	$result = $stmt->fetchAll(PDO::FETCH_ASSOC);
	echo "<table  width=100% border=1 cellspacing=1> \n";
	echo "<tr>\n";
	echo "<th>詳細・印刷</th><th>入力・編集</th><th>管轄</th><th>防火対象物名</th><th>令別表</th><th>状況</th><th>不備その他内容</th><th>立入検査日</th><th>主担当名</th><th>副担当名</th><th>改修計画提出期限</th><th>改修計画提出日</th><th>改修計画完了期限</th><th>改修報告提出日</th><th>最終指導日</th><th></th>\n";
	echo "</tr>\n";
	foreach ($result as $row) {
		...
		echo "<a href=../shidou/shidou_listedit.php?id=" . htmlspecialchars($row['id_kensa'],ENT_QUOTES,'UTF-8') . ">指導内容</a>\n";
		echo "</td>\n";
...
//		echo "<td>" . htmlspecialchars($row['shokuinbangou1'],ENT_QUOTES,'UTF-8') . "</td>\n";
		...
		echo "<td>\n";
		echo "<a href=../kensapredelete.php?id=" . htmlspecialchars($row['id_kensa'],ENT_QUOTES,'UTF-8') . ">削除</a>\n";
		echo "</td>\n";
